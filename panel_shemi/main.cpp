...
//   Nothing in this file except the test number. The whole change is
//   in platformio.ini: upload_speed dropped from 921600 to 115200,
//   and the port pinned to COM11 instead of being auto-detected.
//
//   TEST 001 compiled cleanly - RAM 22.4 percent, Flash 8.0 percent -
//   and failed only at the upload step with "No serial data
//   received". Disconnecting the board made COM11 vanish from Device
//   Manager, which proves the port is the board and the driver is
//   fine. What was left was the baud handshake.
//
// WHY TEST 002 AND NOT 082
...
//   TEST 081 stays in the repo as the reference for the UI that has
//   to be ported afterwards.
//
...
//
// IT WILL NOT BUILD WITHOUT
//   boards\ESP32-S3-Touch-LCD-7B.json
//   lib\   the whole folder from the Waveshare demo,
//          PlatformIO/13_LVGL_TRANSPLANT/lib
//   Both directly under panel_shemi, as siblings. lib inside boards
//   does not work - PlatformIO only scans lib at the project root.
//
//   Their port layer is used verbatim on purpose. It carries the
//   panel timings, the RGB pin map and the GT911 setup, all proven
//   on this exact board. Known-good beats deduced.
//
// WHAT CARRIES OVER FROM panel_ira
//   The touch controller is the same GT911, confirmed from the chip
//   marking on the touch flex. So: five touch points, not ten.
//   Wire.endTransmission(false) for repeated-start reads.
...
//   measure about 17 fps on the LVGL benchmark at a 30 MHz pixel
//   clock. Judge the UI against that, not against the old panel.
//
...
static void buildScreen() {
  ...
  lv_label_set_text(title, "PANEL SHEMI  -  TEST 002");
  ...
  lv_slider_set_range(sl, 5, 100);            // never to zero, or the
  lv_slider_set_value(sl, 100, LV_ANIM_OFF);  // screen looks dead
...
void setup() {
  ...
  Serial.println();
  Serial.println("=== PANEL SHEMI - TEST 002 - Waveshare 7B bring-up ===");
